...
xArmServoController::xArmServoController(xArmMode mode, uart_inst_t * uart)  {
  xMode = mode;
  pUart = uart;
}
...
void xArmServoController::send(int cmd, int len)
{
	/*
  serial_port.flush();
  ...
  */
	uart_putc_raw ( pUart,  SIGNATURE);
	uart_putc_raw ( pUart,  SIGNATURE);
	uart_putc_raw ( pUart,  len + 2);
	uart_putc_raw ( pUart,  cmd);
	if (len > 0) {
		uart_write_blocking(pUart, _buffer, len);
	}
...
int xArmServoController::recv(int cmd)
{
  ...
  return -1;
  */
	uint32_t timeout = to_ms_since_boot ( get_absolute_time()) + TIMEOUT;

	while ( timeout > to_ms_since_boot ( get_absolute_time())){
		if ( uart_is_readable(pUart)){
			uart_read_blocking (pUart,  _buffer, 4);
			//debugPrintBuffer(" read",   _buffer,  4);
		    if (_buffer[0] == SIGNATURE && _buffer[1] == SIGNATURE && _buffer[3] == cmd) {
			    int len = _buffer[2] - 2;
			    uart_read_blocking (pUart,  _buffer, len);
			    //debugPrintBuffer(" read",   _buffer,  len);
			    return len;
			} else {
				printf("READ FAILED\n");
				return -1;
...
void xArmServoController::setPosition(int servo_id, unsigned position, unsigned duration , bool wait)
{
...
void xArmServoController::setPosition(xArmServo servo, unsigned duration , bool wait )
{
  setPosition(servo.servo_id, servo.position, duration, wait);
}

void xArmServoController::setPosition(xArmServo servos[], int count, unsigned duration , bool wait)
{
  int len = count * 3 + 3;
  _buffer[0] = count;
  _buffer[1] = lowByte(duration);
...
int xArmServoController::getPosition(int servo_id)
{
  ...
  if (-1 != recv(CMD_GET_SERVO_POSITION)) {
	  //debugPrintBuffer("POS", _buffer, 4);
	  int res = _buffer[3];
	  res = res << 8;
	  res = res | _buffer[2];
	  return res;
...
uart_putc (pUart, CMD_BEEP);
}


uint8_t xArmServoController::lowByte(uint16_t x){
	return (x & 0xFF);
}
uint8_t xArmServoController::highByte(uint16_t x){
	return ((x >> 8) & 0xFF);
}
 ...
 */
void xArmServoController::debugPrintBuffer(const char *title, const void * pBuffer, size_t bytes){
	size_t count =0;
	size_t lineEnd=0;
